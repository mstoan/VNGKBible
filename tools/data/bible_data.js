var BOOK_DATA = [ //Số đoạn và câu
	{
		"name": "Front matter",
		"sortOrder": 0,
		"dbsCode": "FR",
		"usfm": "FRT",
		"osis": "Preface",
		"chapters": [],
		"chapterCount": 1,
		"names": {
			"eng": [
				"Front matter"
			]
		},
		"testament": "x"
	},
	{
		"name": "Introduction",
		"sortOrder": 1,
		"dbsCode": "IN",
		"usfm": "INT",
		"osis": "Intro",
		"chapters": [],
		"chapterCount": 1,
		"names": {
			"eng": [
				"Introduction"
			]
		},
		"testament": "x"
	},
	{
		"name": "Genesis",
		"sortOrder": 2,
		"dbsCode": "GN",
		"usfm": "GEN",
		"osis": "Gen",
		"chapters": [
			31,
			25,
			24,
			26,
			32,
			22,
			24,
			22,
			29,
			32,
			32,
			20,
			18,
			24,
			21,
			16,
			27,
			33,
			38,
			18,
			34,
			24,
			20,
			67,
			34,
			35,
			46,
			22,
			35,
			43,
			55,
			32,
			20,
			31,
			29,
			43,
			36,
			30,
			23,
			23,
			57,
			38,
			34,
			34,
			28,
			34,
			31,
			22,
			33,
			26
		],
		"chapterCount": 50,
		"names": {
			"eng": [
				"Genesis",
				"Ge",
				"Gen"
			]
		},
		"testament": "o",
		"unboundCode": "01O"
	},
	{
		"name": "Exodus",
		"sortOrder": 3,
		"dbsCode": "EX",
		"usfm": "EXO",
		"osis": "Exod",
		"chapters": [
			22,
			25,
			22,
			31,
			23,
			30,
			25,
			32,
			35,
			29,
			10,
			51,
			22,
			31,
			27,
			36,
			16,
			27,
			25,
			26,
			36,
			31,
			33,
			18,
			40,
			37,
			21,
			43,
			46,
			38,
			18,
			35,
			23,
			35,
			35,
			38,
			29,
			31,
			43,
			38
		],
		"chapterCount": 40,
		"names": {
			"eng": [
				"Exodus",
				"Ex",
				"Exo"
			]
		},
		"testament": "o",
		"unboundCode": "02O"
	},
	{
		"name": "Leviticus",
		"sortOrder": 4,
		"dbsCode": "LV",
		"usfm": "LEV",
		"osis": "Lev",
		"chapters": [
			17,
			16,
			17,
			35,
			19,
			30,
			38,
			36,
			24,
			20,
			47,
			8,
			59,
			57,
			33,
			34,
			16,
			30,
			37,
			27,
			24,
			33,
			44,
			23,
			55,
			46,
			34
		],
		"chapterCount": 27,
		"names": {
			"eng": [
				"Leviticus",
				"Le",
				"Lev"
			]
		},
		"testament": "o",
		"unboundCode": "03O"
	},
	{
		"name": "Numbers",
		"sortOrder": 5,
		"dbsCode": "NU",
		"usfm": "NUM",
		"osis": "Num",
		"chapters": [
			54,
			34,
			51,
			49,
			31,
			27,
			89,
			26,
			23,
			36,
			35,
			16,
			33,
			45,
			41,
			50,
			13,
			32,
			22,
			29,
			35,
			41,
			30,
			25,
			18,
			65,
			23,
			31,
			40,
			16,
			54,
			42,
			56,
			29,
			34,
			13
		],
		"chapterCount": 36,
		"names": {
			"eng": [
				"Numbers",
				"Nu",
				"Num"
			]
		},
		"testament": "o",
		"unboundCode": "04O"
	},
	{
		"name": "Deuteronomy",
		"sortOrder": 6,
		"dbsCode": "DT",
		"usfm": "DEU",
		"osis": "Deut",
		"chapters": [
			46,
			37,
			29,
			49,
			33,
			25,
			26,
			20,
			29,
			22,
			32,
			32,
			18,
			29,
			23,
			22,
			20,
			22,
			21,
			20,
			23,
			30,
			25,
			22,
			19,
			19,
			26,
			68,
			29,
			20,
			30,
			52,
			29,
			12
		],
		"chapterCount": 34,
		"names": {
			"eng": [
				"Deuteronomy",
				"Dt",
				"Deut",
				"Deu",
				"De"
			]
		},
		"testament": "o",
		"unboundCode": "05O"
	},
	{
		"name": "Joshua",
		"sortOrder": 7,
		"dbsCode": "JS",
		"usfm": "JOS",
		"osis": "Josh",
		"chapters": [
			18,
			24,
			17,
			24,
			15,
			27,
			26,
			35,
			27,
			43,
			23,
			24,
			33,
			15,
			63,
			10,
			18,
			28,
			51,
			9,
			45,
			34,
			16,
			33
		],
		"chapterCount": 24,
		"names": {
			"eng": [
				"Joshua",
				"Js",
				"Jos",
				"Jos",
				"Josh"
			]
		},
		"testament": "o",
		"unboundCode": "06O"
	},
	{
		"name": "Judges",
		"sortOrder": 8,
		"dbsCode": "JG",
		"usfm": "JDG",
		"osis": "Judg",
		"chapters": [
			36,
			23,
			31,
			24,
			31,
			40,
			25,
			35,
			57,
			18,
			40,
			15,
			25,
			20,
			20,
			31,
			13,
			31,
			30,
			48,
			25
		],
		"chapterCount": 21,
		"names": {
			"eng": [
				"Judges",
				"Jg",
				"Jdg",
				"Jdgs"
			]
		},
		"testament": "o",
		"unboundCode": "07O"
	},
	{
		"name": "Ruth",
		"sortOrder": 9,
		"dbsCode": "RT",
		"usfm": "RUT",
		"osis": "Ruth",
		"chapters": [
			22,
			23,
			18,
			22
		],
		"chapterCount": 4,
		"names": {
			"eng": [
				"Ruth",
				"Ru",
				"Rut"
			]
		},
		"testament": "o",
		"unboundCode": "08O"
	},
	{
		"name": "The First Book of Samuel",
		"sortOrder": 10,
		"dbsCode": "S1",
		"usfm": "1SA",
		"osis": "1Sam",
		"chapters": [
			28,
			36,
			21,
			22,
			12,
			21,
			17,
			22,
			27,
			27,
			15,
			25,
			23,
			52,
			35,
			23,
			58,
			30,
			24,
			42,
			15,
			23,
			29,
			22,
			44,
			25,
			12,
			25,
			11,
			31,
			13
		],
		"chapterCount": 31,
		"names": {
			"eng": [
				"1 Samuel",
				"1S",
				"1 Sam",
				"1Sam",
				"1 Sa",
				"1Sa",
				"I Samuel",
				"I Sam",
				"I Sa"
			]
		},
		"testament": "o",
		"unboundCode": "09O"
	},
	{
		"name": "The Second Book of Samuel",
		"sortOrder": 11,
		"dbsCode": "S2",
		"usfm": "2SA",
		"osis": "2Sam",
		"chapters": [
			27,
			32,
			39,
			12,
			25,
			23,
			29,
			18,
			13,
			19,
			27,
			31,
			39,
			33,
			37,
			23,
			29,
			33,
			43,
			26,
			22,
			51,
			39,
			25
		],
		"chapterCount": 24,
		"names": {
			"eng": [
				"2 Samuel",
				"2S",
				"2 Sam",
				"2Sam",
				"2 Sa",
				"2Sa",
				"II Samuel",
				"II Sam",
				"II Sa",
				"IIS"
			]
		},
		"testament": "o",
		"unboundCode": "10O"
	},
	{
		"name": "The First Book of Kings",
		"sortOrder": 12,
		"dbsCode": "K1",
		"usfm": "1KI",
		"osis": "1Kgs",
		"chapters": [
			53,
			46,
			28,
			34,
			18,
			38,
			51,
			66,
			28,
			29,
			43,
			33,
			34,
			31,
			34,
			34,
			24,
			46,
			21,
			43,
			29,
			53
		],
		"chapterCount": 22,
		"names": {
			"eng": [
				"1 Kings",
				"1K",
				"1 Kin",
				"1Kin",
				"1 Ki",
				"IK",
				"1Ki",
				"I Kings",
				"I Kin",
				"I Ki"
			]
		},
		"testament": "o",
		"unboundCode": "11O"
	},
	{
		"name": "The Second Book of Kings",
		"sortOrder": 13,
		"dbsCode": "K2",
		"usfm": "2KI",
		"osis": "2Kgs",
		"chapters": [
			18,
			25,
			27,
			44,
			27,
			33,
			20,
			29,
			37,
			36,
			21,
			21,
			25,
			29,
			38,
			20,
			41,
			37,
			37,
			21,
			26,
			20,
			37,
			20,
			30
		],
		"chapterCount": 25,
		"names": {
			"eng": [
				"2 Kings",
				"2K",
				"2 Kin",
				"2Kin",
				"2 Ki",
				"IIK",
				"2Ki",
				"II Kings",
				"II Kin",
				"II Ki"
			]
		},
		"testament": "o",
		"unboundCode": "12O"
	},
	{
		"name": "The First Book of Chronicles",
		"sortOrder": 14,
		"dbsCode": "R1",
		"usfm": "1CH",
		"osis": "1Chr",
		"chapters": [
			54,
			55,
			24,
			43,
			26,
			81,
			40,
			40,
			44,
			14,
			47,
			40,
			14,
			17,
			29,
			43,
			27,
			17,
			19,
			8,
			30,
			19,
			32,
			31,
			31,
			32,
			34,
			21,
			30
		],
		"chapterCount": 29,
		"names": {
			"eng": [
				"1 Chronicles",
				"1Ch",
				"1 Chr",
				"1Chr",
				"1 Ch",
				"ICh",
				"I Chronicles",
				"I Chr",
				"I Ch"
			]
		},
		"testament": "o",
		"unboundCode": "13O"
	},
	{
		"name": "The Second Book of Chronicles",
		"sortOrder": 15,
		"dbsCode": "R2",
		"usfm": "2CH",
		"osis": "2Chr",
		"chapters": [
			17,
			18,
			17,
			22,
			14,
			42,
			22,
			18,
			31,
			19,
			23,
			16,
			22,
			15,
			19,
			14,
			19,
			34,
			11,
			37,
			20,
			12,
			21,
			27,
			28,
			23,
			9,
			27,
			36,
			27,
			21,
			33,
			25,
			33,
			27,
			23
		],
		"chapterCount": 36,
		"names": {
			"eng": [
				"2 Chronicles",
				"2Ch",
				"2 Chr",
				"2 Chr",
				"2Chr",
				"2 Ch",
				"IICh",
				"II Chronicles",
				"II Chr",
				"II Ch"
			]
		},
		"testament": "o",
		"unboundCode": "14O"
	},
	{
		"name": "Ezra",
		"sortOrder": 16,
		"dbsCode": "ER",
		"usfm": "EZR",
		"osis": "Ezra",
		"chapters": [
			11,
			70,
			13,
			24,
			17,
			22,
			28,
			36,
			15,
			44
		],
		"chapterCount": 10,
		"names": {
			"eng": [
				"Ezra",
				"Ezr"
			]
		},
		"testament": "o",
		"unboundCode": "15O"
	},
	{
		"name": "Nehemiah",
		"sortOrder": 17,
		"dbsCode": "NH",
		"usfm": "NEH",
		"osis": "Neh",
		"chapters": [
			11,
			20,
			32,
			23,
			19,
			19,
			73,
			18,
			38,
			39,
			36,
			47,
			31
		],
		"chapterCount": 13,
		"names": {
			"eng": [
				"Nehemiah",
				"Ne",
				"Neh",
				"Neh",
				"Ne"
			]
		},
		"testament": "o",
		"unboundCode": "16O"
	},
	{
		"name": "Esther",
		"sortOrder": 18,
		"dbsCode": "ET",
		"usfm": "EST",
		"osis": "Esth",
		"chapters": [
			22,
			23,
			15,
			17,
			14,
			14,
			10,
			17,
			32,
			3
		],
		"chapterCount": 10,
		"names": {
			"eng": [
				"Esther",
				"Es",
				"Est",
				"Esth"
			]
		},
		"testament": "o",
		"unboundCode": "17O"
	},
	{
		"name": "Job",
		"sortOrder": 19,
		"dbsCode": "JB",
		"usfm": "JOB",
		"osis": "Job",
		"chapters": [
			22,
			13,
			26,
			21,
			27,
			30,
			21,
			22,
			35,
			22,
			20,
			25,
			28,
			22,
			35,
			22,
			16,
			21,
			29,
			29,
			34,
			30,
			17,
			25,
			6,
			14,
			23,
			28,
			25,
			31,
			40,
			22,
			33,
			37,
			16,
			33,
			24,
			41,
			30,
			24,
			34,
			17
		],
		"chapterCount": 42,
		"names": {
			"eng": [
				"Job",
				"Jb",
				"Job"
			]
		},
		"testament": "o",
		"unboundCode": "18O"
	},
	{
		"name": "Psalms",
		"sortOrder": 20,
		"dbsCode": "PS",
		"usfm": "PSA",
		"osis": "Ps",
		"chapters": [
			6,
			12,
			8,
			8,
			12,
			10,
			17,
			9,
			20,
			18,
			7,
			8,
			6,
			7,
			5,
			11,
			15,
			50,
			14,
			9,
			13,
			31,
			6,
			10,
			22,
			12,
			14,
			9,
			11,
			12,
			24,
			11,
			22,
			22,
			28,
			12,
			40,
			22,
			13,
			17,
			13,
			11,
			5,
			26,
			17,
			11,
			9,
			14,
			20,
			23,
			19,
			9,
			6,
			7,
			23,
			13,
			11,
			11,
			17,
			12,
			8,
			12,
			11,
			10,
			13,
			20,
			7,
			35,
			36,
			5,
			24,
			20,
			28,
			23,
			10,
			12,
			20,
			72,
			13,
			19,
			16,
			8,
			18,
			12,
			13,
			17,
			7,
			18,
			52,
			17,
			16,
			15,
			5,
			23,
			11,
			13,
			12,
			9,
			9,
			5,
			8,
			28,
			22,
			35,
			45,
			48,
			43,
			13,
			31,
			7,
			10,
			10,
			9,
			8,
			18,
			19,
			2,
			29,
			176,
			7,
			8,
			9,
			4,
			8,
			5,
			6,
			5,
			6,
			8,
			8,
			3,
			18,
			3,
			3,
			21,
			26,
			9,
			8,
			24,
			13,
			10,
			7,
			12,
			15,
			21,
			10,
			20,
			14,
			9,
			6
		],
		"chapterCount": 150,
		"names": {
			"eng": [
				"Psalm",
				"Ps",
				"Psa"
			]
		},
		"testament": "o",
		"unboundCode": "19O"
	},
	{
		"name": "Proverbs",
		"sortOrder": 21,
		"dbsCode": "PR",
		"usfm": "PRO",
		"osis": "Prov",
		"chapters": [
			33,
			22,
			35,
			27,
			23,
			35,
			27,
			36,
			18,
			32,
			31,
			28,
			25,
			35,
			33,
			33,
			28,
			24,
			29,
			30,
			31,
			29,
			35,
			34,
			28,
			28,
			27,
			28,
			27,
			33,
			31
		],
		"chapterCount": 31,
		"names": {
			"eng": [
				"Proverbs",
				"Pr",
				"Prov",
				"Pro"
			]
		},
		"testament": "o",
		"unboundCode": "20O"
	},
	{
		"name": "Ecclesiastes",
		"sortOrder": 22,
		"dbsCode": "EC",
		"usfm": "ECC",
		"osis": "Eccl",
		"chapters": [
			18,
			26,
			22,
			16,
			20,
			12,
			29,
			17,
			18,
			20,
			10,
			14
		],
		"chapterCount": 12,
		"names": {
			"eng": [
				"Ecclesiastes",
				"Ec",
				"Ecc",
				"Qohelet"
			]
		},
		"testament": "o",
		"unboundCode": "21O"
	},
	{
		"name": "Song of Solomon",
		"sortOrder": 23,
		"dbsCode": "SS",
		"usfm": "SNG",
		"osis": "Song",
		"chapters": [
			17,
			17,
			11,
			16,
			16,
			13,
			13,
			14
		],
		"chapterCount": 8,
		"names": {
			"eng": [
				"Song of Songs",
				"So",
				"Sos",
				"Song of Solomon",
				"SOS",
				"SongOfSongs",
				"SongofSolomon",
				"Canticle of Canticles"
			]
		},
		"testament": "o",
		"unboundCode": "22O"
	},
	{
		"name": "Isaiah",
		"sortOrder": 24,
		"dbsCode": "IS",
		"usfm": "ISA",
		"osis": "Isa",
		"chapters": [
			31,
			22,
			26,
			6,
			30,
			13,
			25,
			22,
			21,
			34,
			16,
			6,
			22,
			32,
			9,
			14,
			14,
			7,
			25,
			6,
			17,
			25,
			18,
			23,
			12,
			21,
			13,
			29,
			24,
			33,
			9,
			20,
			24,
			17,
			10,
			22,
			38,
			22,
			8,
			31,
			29,
			25,
			28,
			28,
			25,
			13,
			15,
			22,
			26,
			11,
			23,
			15,
			12,
			17,
			13,
			12,
			21,
			14,
			21,
			22,
			11,
			12,
			19,
			12,
			25,
			24
		],
		"chapterCount": 66,
		"names": {
			"eng": [
				"Isaiah",
				"Is",
				"Isa"
			]
		},
		"testament": "o",
		"unboundCode": "23O"
	},
	{
		"name": "Jeremiah",
		"sortOrder": 25,
		"dbsCode": "JR",
		"usfm": "JER",
		"osis": "Jer",
		"chapters": [
			19,
			37,
			25,
			31,
			31,
			30,
			34,
			22,
			26,
			25,
			23,
			17,
			27,
			22,
			21,
			21,
			27,
			23,
			15,
			18,
			14,
			30,
			40,
			10,
			38,
			24,
			22,
			17,
			32,
			24,
			40,
			44,
			26,
			22,
			19,
			32,
			21,
			28,
			18,
			16,
			18,
			22,
			13,
			30,
			5,
			28,
			7,
			47,
			39,
			46,
			64,
			34
		],
		"chapterCount": 52,
		"names": {
			"eng": [
				"Jeremiah",
				"Je",
				"Jer"
			]
		},
		"testament": "o",
		"unboundCode": "24O"
	},
	{
		"name": "Lamentations",
		"sortOrder": 26,
		"dbsCode": "LM",
		"usfm": "LAM",
		"osis": "Lam",
		"chapters": [
			22,
			22,
			66,
			22,
			22
		],
		"chapterCount": 5,
		"names": {
			"eng": [
				"Lamentations",
				"La",
				"Lam",
				"Lament"
			]
		},
		"testament": "o",
		"unboundCode": "25O"
	},
	{
		"name": "Ezekiel",
		"sortOrder": 27,
		"dbsCode": "EK",
		"usfm": "EZK",
		"osis": "Ezek",
		"chapters": [
			28,
			10,
			27,
			17,
			17,
			14,
			27,
			18,
			11,
			22,
			25,
			28,
			23,
			23,
			8,
			63,
			24,
			32,
			14,
			49,
			32,
			31,
			49,
			27,
			17,
			21,
			36,
			26,
			21,
			26,
			18,
			32,
			33,
			31,
			15,
			38,
			28,
			23,
			29,
			49,
			26,
			20,
			27,
			31,
			25,
			24,
			23,
			35
		],
		"chapterCount": 48,
		"names": {
			"eng": [
				"Ezekiel",
				"Ek",
				"Ezek",
				"Eze"
			]
		},
		"testament": "o",
		"unboundCode": "26O"
	},
	{
		"name": "Daniel",
		"sortOrder": 28,
		"dbsCode": "DN",
		"usfm": "DAN",
		"osis": "Dan",
		"chapters": [
			21,
			49,
			30,
			37,
			31,
			28,
			28,
			27,
			27,
			21,
			45,
			13
		],
		"chapterCount": 12,
		"names": {
			"eng": [
				"Daniel",
				"Da",
				"Dan",
				"Dl",
				"Dnl"
			]
		},
		"testament": "o",
		"unboundCode": "27O"
	},
	{
		"name": "Hosea",
		"sortOrder": 29,
		"dbsCode": "HS",
		"usfm": "HOS",
		"osis": "Hos",
		"chapters": [
			11,
			23,
			5,
			19,
			15,
			11,
			16,
			14,
			17,
			15,
			12,
			14,
			16,
			9
		],
		"chapterCount": 14,
		"names": {
			"eng": [
				"Hosea",
				"Ho",
				"Hos"
			]
		},
		"testament": "o",
		"unboundCode": "28O"
	},
	{
		"name": "Joel",
		"sortOrder": 30,
		"dbsCode": "JL",
		"usfm": "JOL",
		"osis": "Joel",
		"chapters": [
			20,
			32,
			21
		],
		"chapterCount": 3,
		"names": {
			"eng": [
				"Joel",
				"Jl",
				"Joel",
				"Joe"
			]
		},
		"testament": "o",
		"unboundCode": "29O"
	},
	{
		"name": "Amos",
		"sortOrder": 31,
		"dbsCode": "AM",
		"usfm": "AMO",
		"osis": "Amos",
		"chapters": [
			15,
			16,
			15,
			13,
			27,
			14,
			17,
			14,
			15
		],
		"chapterCount": 9,
		"names": {
			"eng": [
				"Amos",
				"Am",
				"Amos",
				"Amo"
			]
		},
		"testament": "o",
		"unboundCode": "30O"
	},
	{
		"name": "Obadiah",
		"sortOrder": 32,
		"dbsCode": "OB",
		"usfm": "OBA",
		"osis": "Obad",
		"chapters": [
			21
		],
		"chapterCount": 1,
		"names": {
			"eng": [
				"Obadiah",
				"Ob",
				"Oba",
				"Obd",
				"Odbh"
			]
		},
		"testament": "o",
		"unboundCode": "31O"
	},
	{
		"name": "Jonah",
		"sortOrder": 33,
		"dbsCode": "JH",
		"usfm": "JON",
		"osis": "Jonah",
		"chapters": [
			17,
			10,
			10,
			11
		],
		"chapterCount": 4,
		"names": {
			"eng": [
				"Jonah",
				"Jh",
				"Jon",
				"Jnh"
			]
		},
		"testament": "o",
		"unboundCode": "32O"
	},
	{
		"name": "Micah",
		"sortOrder": 34,
		"dbsCode": "MC",
		"usfm": "MIC",
		"osis": "Mic",
		"chapters": [
			16,
			13,
			12,
			13,
			15,
			16,
			20
		],
		"chapterCount": 7,
		"names": {
			"eng": [
				"Micah",
				"Mi",
				"Mic"
			]
		},
		"testament": "o",
		"unboundCode": "33O"
	},
	{
		"name": "Nahum",
		"sortOrder": 35,
		"dbsCode": "NM",
		"usfm": "NAM",
		"osis": "Nah",
		"chapters": [
			15,
			13,
			19
		],
		"chapterCount": 3,
		"names": {
			"eng": [
				"Nahum",
				"Na",
				"Nah",
				"Nah",
				"Na"
			]
		},
		"testament": "o",
		"unboundCode": "34O"
	},
	{
		"name": "Habakkuk",
		"sortOrder": 36,
		"dbsCode": "HK",
		"usfm": "HAB",
		"osis": "Hab",
		"chapters": [
			17,
			20,
			19
		],
		"chapterCount": 3,
		"names": {
			"eng": [
				"Habakkuk",
				"Hb",
				"Hab",
				"Hk",
				"Habk"
			]
		},
		"testament": "o",
		"unboundCode": "35O"
	},
	{
		"name": "Zephaniah",
		"sortOrder": 37,
		"dbsCode": "ZP",
		"usfm": "ZEP",
		"osis": "Zeph",
		"chapters": [
			18,
			15,
			20
		],
		"chapterCount": 3,
		"names": {
			"eng": [
				"Zephaniah",
				"Zp",
				"Zep",
				"Zeph"
			]
		},
		"testament": "o",
		"unboundCode": "36O"
	},
	{
		"name": "Haggai",
		"sortOrder": 38,
		"dbsCode": "HG",
		"usfm": "HAG",
		"osis": "Hag",
		"chapters": [
			15,
			23
		],
		"chapterCount": 2,
		"names": {
			"eng": [
				"Haggai",
				"Ha",
				"Hag",
				"Hagg"
			]
		},
		"testament": "o",
		"unboundCode": "37O"
	},
	{
		"name": "Zechariah",
		"sortOrder": 39,
		"dbsCode": "ZC",
		"usfm": "ZEC",
		"osis": "Zech",
		"chapters": [
			21,
			13,
			10,
			14,
			11,
			15,
			14,
			23,
			17,
			12,
			17,
			14,
			9,
			21
		],
		"chapterCount": 14,
		"names": {
			"eng": [
				"Zechariah",
				"Zc",
				"Zech",
				"Zec"
			]
		},
		"testament": "o",
		"unboundCode": "38O"
	},
	{
		"name": "Malachi",
		"sortOrder": 40,
		"dbsCode": "ML",
		"usfm": "MAL",
		"osis": "Mal",
		"chapters": [
			14,
			17,
			18,
			6
		],
		"chapterCount": 4,
		"names": {
			"eng": [
				"Malachi",
				"Ml",
				"Mal",
				"Mlc"
			]
		},
		"testament": "o",
		"unboundCode": "39O"
	},
	{
		"name": "Tobit",
		"sortOrder": 41,
		"dbsCode": "TB",
		"usfm": "TOB",
		"osis": "Tob",
		"chapters": [
			22,
			14,
			17,
			21,
			22,
			17,
			18
		],
		"chapterCount": 14,
		"names": {
			"eng": [
				"Tobit"
			]
		},
		"testament": "a",
		"unboundCode": "67A"
	},
	{
		"name": "Judith",
		"sortOrder": 42,
		"dbsCode": "JT",
		"usfm": "JDT",
		"osis": "Jdt",
		"chapters": [],
		"chapterCount": 16,
		"names": {
			"eng": [
				"Judith"
			]
		},
		"testament": "a",
		"unboundCode": "68A"
	},
	{
		"name": "Esther (Greek)",
		"sortOrder": 43,
		"dbsCode": "EG",
		"usfm": "ESG",
		"osis": "EsthGr",
		"chapters": [],
		"chapterCount": 16,
		"names": {
			"eng": [
				"Esther (Greek)"
			]
		},
		"testament": "a"
	},
	{
		"name": "Additions to Esther",
		"sortOrder": 44,
		"dbsCode": "AE",
		"usfm": "ADE",
		"osis": "AddEsth",
		"chapters": [],
		"chapterCount": 16,
		"names": {
			"eng": [
				"Additions to Esther"
			]
		},
		"testament": "a",
		"unboundCode": "69A"
	},
	{
		"name": "Wisdom of Solomon",
		"sortOrder": 45,
		"dbsCode": "WS",
		"usfm": "WIS",
		"osis": "Wis",
		"chapters": [],
		"chapterCount": 19,
		"names": {
			"eng": [
				"Wisdom",
				"Wisdom of Solomon"
			]
		},
		"testament": "a",
		"unboundCode": "70A"
	},
	{
		"name": "Sirach",
		"sortOrder": 46,
		"dbsCode": "SR",
		"usfm": "SIR",
		"osis": "Sir",
		"chapters": [],
		"chapterCount": 51,
		"names": {
			"eng": [
				"Sirach",
				"Ecclesiasticus"
			]
		},
		"testament": "a",
		"unboundCode": "71A"
	},
	{
		"name": "Baruch",
		"sortOrder": 47,
		"dbsCode": "BR",
		"usfm": "BAR",
		"osis": "Bar",
		"chapters": [],
		"chapterCount": 5,
		"names": {
			"eng": [
				"Baruch"
			]
		},
		"testament": "a",
		"unboundCode": "72A"
	},
	{
		"name": "Letter of Jeremiah",
		"sortOrder": 48,
		"dbsCode": "LJ",
		"usfm": "LJE",
		"osis": "EpJer",
		"chapters": [],
		"chapterCount": 1,
		"names": {
			"eng": [
				"Letter of Jeremiah"
			]
		},
		"testament": "a",
		"unboundCode": "73A"
	},
	{
		"name": "Song of the Three Children",
		"sortOrder": 49,
		"dbsCode": "PA",
		"usfm": "S3Y",
		"osis": "PrAzar",
		"chapters": [],
		"chapterCount": 1,
		"names": {
			"eng": [
				"Prayer of Azariah"
			]
		},
		"testament": "a",
		"unboundCode": "74A"
	},
	{
		"name": "Susanna",
		"sortOrder": 50,
		"dbsCode": "SN",
		"usfm": "SUS",
		"osis": "Sus",
		"chapters": [],
		"chapterCount": 1,
		"names": {
			"eng": [
				"Susanna"
			]
		},
		"testament": "a",
		"unboundCode": "75A"
	},
	{
		"name": "Bel and the Dragon",
		"sortOrder": 51,
		"dbsCode": "BL",
		"usfm": "BEL",
		"osis": "Bel",
		"chapters": [],
		"chapterCount": 1,
		"names": {
			"eng": [
				"Bel and the Dragon"
			]
		},
		"testament": "a",
		"unboundCode": "76A"
	},
	{
		"name": "1 Maccabees",
		"sortOrder": 52,
		"dbsCode": "M1",
		"usfm": "1MA",
		"osis": "1Macc",
		"chapters": [],
		"chapterCount": 16,
		"names": {
			"eng": [
				"1 Maccabees"
			]
		},
		"testament": "a",
		"unboundCode": "77A"
	},
	{
		"name": "2 Maccabees",
		"sortOrder": 53,
		"dbsCode": "M2",
		"usfm": "2MA",
		"osis": "2Macc",
		"chapters": [],
		"chapterCount": 15,
		"names": {
			"eng": [
				"2 Maccabees"
			]
		},
		"testament": "a",
		"unboundCode": "78A"
	},
	{
		"name": "1 Esdras",
		"sortOrder": 54,
		"dbsCode": "E1",
		"usfm": "1ES",
		"osis": "1Esd",
		"chapters": [],
		"chapterCount": 9,
		"names": {
			"eng": [
				"1 Esdras"
			]
		},
		"testament": "a",
		"unboundCode": "81A"
	},
	{
		"name": "Prayer of Manasses",
		"sortOrder": 55,
		"dbsCode": "PN",
		"usfm": "MAN",
		"osis": "PrMan",
		"chapters": [],
		"chapterCount": 1,
		"names": {
			"eng": [
				"Prayer of Manasseh",
				"Song of the Three Children"
			]
		},
		"testament": "a",
		"unboundCode": "83A"
	},
	{
		"name": "Psalm 151",
		"sortOrder": 56,
		"dbsCode": "PX",
		"usfm": "PS2",
		"osis": "AddPs",
		"chapters": [],
		"chapterCount": 1,
		"names": {
			"eng": [
				"Psalm 151"
			]
		},
		"testament": "a"
	},
	{
		"name": "3 Maccabees",
		"sortOrder": 57,
		"dbsCode": "M3",
		"usfm": "3MA",
		"osis": "3Macc",
		"chapters": [],
		"chapterCount": 7,
		"names": {
			"eng": [
				"3 Maccabees"
			]
		},
		"testament": "a",
		"unboundCode": "79A"
	},
	{
		"name": "2 Esdras",
		"sortOrder": 58,
		"dbsCode": "E2",
		"usfm": "2ES",
		"osis": "2Esd",
		"chapters": [],
		"chapterCount": 16,
		"names": {
			"eng": [
				"2 Esdras",
				"5 Ezra"
			]
		},
		"testament": "a",
		"unboundCode": "82A"
	},
	{
		"name": "4 Maccabees",
		"sortOrder": 59,
		"dbsCode": "M4",
		"usfm": "4MA",
		"osis": "4Macc",
		"chapters": [],
		"chapterCount": 18,
		"names": {
			"eng": [
				"4 Maccabees"
			]
		},
		"testament": "a",
		"unboundCode": "80A"
	},
	{
		"name": "Odes of Solomon",
		"sortOrder": 60,
		"dbsCode": "OS",
		"usfm": "ODS",
		"osis": "OdesSol",
		"chapters": [],
		"chapterCount": 0,
		"names": {
			"eng": [
				"Odes of Solomon"
			]
		},
		"testament": "Septuagint"
	},
	{
		"name": "Psalms of Solomon",
		"sortOrder": 61,
		"dbsCode": "SP",
		"usfm": "PSS",
		"osis": "PssSol",
		"chapters": [],
		"chapterCount": 0,
		"names": {
			"eng": [
				"Psalms of Solomon"
			]
		},
		"testament": "Septuagint"
	},
	{
		"name": "Epistle to the Laodiceans",
		"sortOrder": 62,
		"dbsCode": "LL",
		"usfm": "EPL",
		"osis": "EpLao",
		"chapters": [],
		"chapterCount": 0,
		"names": {
			"eng": [
				"Epistle to the Laodiceans"
			]
		},
		"testament": "Vulgate"
	},
	{
		"name": "Ethiopic Apocalypse of Enoch",
		"sortOrder": 63,
		"dbsCode": "N1",
		"usfm": "1EN",
		"osis": "1En",
		"chapters": [],
		"chapterCount": 0,
		"names": {
			"eng": [
				"Ethiopic Apocalypse of Enoch"
			]
		},
		"testament": "Orthodox Canon"
	},
	{
		"name": "Jubilees",
		"sortOrder": 64,
		"dbsCode": "JE",
		"usfm": "JUB",
		"osis": "Jub",
		"chapters": [],
		"chapterCount": 0,
		"names": {
			"eng": [
				"Jubilees"
			]
		},
		"testament": "Orthodox Canon"
	},
	{
		"name": "Additions to Daniel",
		"sortOrder": 65,
		"dbsCode": "AD",
		"usfm": "DNT",
		"osis": "AddDan",
		"chapters": [],
		"chapterCount": 14,
		"names": {
			"eng": [
				"Additions to Daniel"
			]
		},
		"testament": "a",
		"unboundCode": ""
	},
	{
		"name": "Daniel (Greek)",
		"sortOrder": 66,
		"dbsCode": "DG",
		"usfm": "DAG",
		"osis": "DanGr",
		"chapters": [],
		"chapterCount": 12,
		"names": {
			"eng": [
				"Daniel (Greek)"
			]
		},
		"testament": "a"
	},
	{
		"name": "The Gospel According to Matthew",
		"sortOrder": 70,
		"dbsCode": "MT",
		"usfm": "MAT",
		"osis": "Matt",
		"chapters": [
			25,
			23,
			17,
			25,
			48,
			34,
			29,
			34,
			38,
			42,
			30,
			50,
			58,
			36,
			39,
			28,
			27,
			35,
			30,
			34,
			46,
			46,
			39,
			51,
			46,
			75,
			66,
			20
		],
		"chapterCount": 28,
		"names": {
			"eng": [
				"Matthew",
				"Mt",
				"Matt",
				"Mat"
			]
		},
		"testament": "n",
		"unboundCode": "40N"
	},
	{
		"name": "The Gospel According to Mark",
		"sortOrder": 71,
		"dbsCode": "MK",
		"usfm": "MRK",
		"osis": "Mark",
		"chapters": [
			45,
			28,
			35,
			41,
			43,
			56,
			37,
			38,
			50,
			52,
			33,
			44,
			37,
			72,
			47,
			20
		],
		"chapterCount": 16,
		"names": {
			"eng": [
				"Mark",
				"Mk",
				"Mar",
				"Mrk"
			]
		},
		"testament": "n",
		"unboundCode": "41N"
	},
	{
		"name": "The Gospel According to Luke",
		"sortOrder": 72,
		"dbsCode": "LK",
		"usfm": "LUK",
		"osis": "Luke",
		"chapters": [
			80,
			52,
			38,
			44,
			39,
			49,
			50,
			56,
			62,
			42,
			54,
			59,
			35,
			35,
			32,
			31,
			37,
			43,
			48,
			47,
			38,
			71,
			56,
			53
		],
		"chapterCount": 24,
		"names": {
			"eng": [
				"Luke",
				"Lk",
				"Luk",
				"Lu"
			]
		},
		"testament": "n",
		"unboundCode": "42N"
	},
	{
		"name": "The Gospel According to John",
		"sortOrder": 73,
		"dbsCode": "JN",
		"usfm": "JHN",
		"osis": "John",
		"chapters": [
			51,
			25,
			36,
			54,
			47,
			71,
			53,
			59,
			41,
			42,
			57,
			50,
			38,
			31,
			27,
			33,
			26,
			40,
			42,
			31,
			25
		],
		"chapterCount": 21,
		"names": {
			"eng": [
				"John",
				"Jn",
				"Joh",
				"Jo"
			]
		},
		"testament": "n",
		"unboundCode": "43N"
	},
	{
		"name": "The Acts of the Apostles",
		"sortOrder": 74,
		"dbsCode": "AC",
		"usfm": "ACT",
		"osis": "Acts",
		"chapters": [
			26,
			47,
			26,
			37,
			42,
			15,
			60,
			40,
			43,
			48,
			30,
			25,
			52,
			28,
			41,
			40,
			34,
			28,
			41,
			38,
			40,
			30,
			35,
			27,
			27,
			32,
			44,
			31
		],
		"chapterCount": 28,
		"names": {
			"eng": [
				"Acts",
				"Ac",
				"Act"
			]
		},
		"testament": "n",
		"unboundCode": "44N"
	},
	{
		"name": "Paul's Letter to the Romans",
		"sortOrder": 75,
		"dbsCode": "RM",
		"usfm": "ROM",
		"osis": "Rom",
		"chapters": [
			32,
			29,
			31,
			25,
			21,
			23,
			25,
			39,
			33,
			21,
			36,
			21,
			14,
			23,
			33,
			27
		],
		"chapterCount": 16,
		"names": {
			"eng": [
				"Romans",
				"Ro",
				"Rom",
				"Rmn",
				"Rmns"
			]
		},
		"testament": "n",
		"unboundCode": "45N"
	},
	{
		"name": "Paul's First Letter to the Corinthians",
		"sortOrder": 76,
		"dbsCode": "C1",
		"usfm": "1CO",
		"osis": "1Cor",
		"chapters": [
			31,
			16,
			23,
			21,
			13,
			20,
			40,
			13,
			27,
			33,
			34,
			31,
			13,
			40,
			58,
			24
		],
		"chapterCount": 16,
		"names": {
			"eng": [
				"1 Corinthians",
				"1Co",
				"1 Cor",
				"1Cor",
				"ICo",
				"1 Co",
				"1Co",
				"I Corinthians",
				"I Cor",
				"I Co"
			]
		},
		"testament": "n",
		"unboundCode": "46N"
	},
	{
		"name": "Paul's Second Lettor to the  Corinthians",
		"sortOrder": 77,
		"dbsCode": "C2",
		"usfm": "2CO",
		"osis": "2Cor",
		"chapters": [
			24,
			17,
			18,
			18,
			21,
			18,
			16,
			24,
			15,
			18,
			33,
			21,
			14
		],
		"chapterCount": 13,
		"names": {
			"eng": [
				"2 Corinthians",
				"2Co",
				"2 Cor",
				"2Cor",
				"IICo",
				"2 Co",
				"2Co",
				"II Corinthians",
				"II Cor",
				"II Co"
			]
		},
		"testament": "n",
		"unboundCode": "47N"
	},
	{
		"name": "Paul's Letter to the Galatians",
		"sortOrder": 78,
		"dbsCode": "GL",
		"usfm": "GAL",
		"osis": "Gal",
		"chapters": [
			24,
			21,
			29,
			31,
			26,
			18
		],
		"chapterCount": 6,
		"names": {
			"eng": [
				"Galatians",
				"Ga",
				"Gal",
				"Gltns"
			]
		},
		"testament": "n",
		"unboundCode": "48N"
	},
	{
		"name": "Paul's Letter to the Ephesians",
		"sortOrder": 79,
		"dbsCode": "EP",
		"usfm": "EPH",
		"osis": "Eph",
		"chapters": [
			23,
			22,
			21,
			32,
			33,
			24
		],
		"chapterCount": 6,
		"names": {
			"eng": [
				"Ephesians",
				"Ep",
				"Eph",
				"Ephn"
			]
		},
		"testament": "n",
		"unboundCode": "49N"
	},
	{
		"name": "Paul's Letter to the Philippians",
		"sortOrder": 80,
		"dbsCode": "PP",
		"usfm": "PHP",
		"osis": "Phil",
		"chapters": [
			30,
			30,
			21,
			23
		],
		"chapterCount": 4,
		"names": {
			"eng": [
				"Philippians",
				"Pp",
				"Phi",
				"Phil",
				"Phi"
			]
		},
		"testament": "n",
		"unboundCode": "50N"
	},
	{
		"name": "Paul's Letter to the Colossians",
		"sortOrder": 81,
		"dbsCode": "CL",
		"usfm": "COL",
		"osis": "Col",
		"chapters": [
			29,
			23,
			25,
			18
		],
		"chapterCount": 4,
		"names": {
			"eng": [
				"Colossians",
				"Co",
				"Col",
				"Colo",
				"Cln",
				"Clns"
			]
		},
		"testament": "n",
		"unboundCode": "51N"
	},
	{
		"name": "Paul's First Letter to the Thessalonians",
		"sortOrder": 82,
		"dbsCode": "H1",
		"usfm": "1TH",
		"osis": "1Thess",
		"chapters": [
			10,
			20,
			13,
			18,
			28
		],
		"chapterCount": 5,
		"names": {
			"eng": [
				"1 Thessalonians",
				"1Th",
				"1 Thess",
				"1Thess",
				"ITh",
				"1 Thes",
				"1Thes",
				"1 The",
				"1The",
				"1 Th",
				"1Th",
				"I Thessalonians",
				"I Thess",
				"I The",
				"I Th"
			]
		},
		"testament": "n",
		"unboundCode": "52N"
	},
	{
		"name": "Paul's Second Letter to the Thessalonians",
		"sortOrder": 83,
		"dbsCode": "H2",
		"usfm": "2TH",
		"osis": "2Thess",
		"chapters": [
			12,
			17,
			18
		],
		"chapterCount": 3,
		"names": {
			"eng": [
				"2 Thessalonians",
				"2Th",
				"2 Thess",
				"2 Thess",
				"2Thess",
				"IITh",
				"2 Thes",
				"2Thes",
				"2 The",
				"2The",
				"2 Th",
				"2Th",
				"II Thessalonians",
				"II Thess",
				"II The",
				"II Th"
			]
		},
		"testament": "n",
		"unboundCode": "53N"
	},
	{
		"name": "Paul's First Letter to Timothy",
		"sortOrder": 84,
		"dbsCode": "T1",
		"usfm": "1TI",
		"osis": "1Tim",
		"chapters": [
			20,
			15,
			16,
			16,
			25,
			21
		],
		"chapterCount": 6,
		"names": {
			"eng": [
				"1 Timothy",
				"1Ti",
				"1 Tim",
				"1Tim",
				"1 Ti",
				"ITi",
				"1Ti",
				"I Timothy",
				"I Tim",
				"I Ti"
			]
		},
		"testament": "n",
		"unboundCode": "54N"
	},
	{
		"name": "Paul's Second Letter to Timothy",
		"sortOrder": 85,
		"dbsCode": "T2",
		"usfm": "2TI",
		"osis": "2Tim",
		"chapters": [
			18,
			26,
			17,
			22
		],
		"chapterCount": 4,
		"names": {
			"eng": [
				"2 Timothy",
				"2Ti",
				"2 Tim",
				"2 Tim",
				"2Tim",
				"2 Ti",
				"IITi",
				"2Ti",
				"II Timothy",
				"II Tim",
				"II Ti"
			]
		},
		"testament": "n",
		"unboundCode": "55N"
	},
	{
		"name": "Paul's Letter to Titus",
		"sortOrder": 86,
		"dbsCode": "TT",
		"usfm": "TIT",
		"osis": "Titus",
		"chapters": [
			16,
			15,
			15
		],
		"chapterCount": 3,
		"names": {
			"eng": [
				"Titus",
				"Ti",
				"Tit",
				"Tt",
				"Ts"
			]
		},
		"testament": "n",
		"unboundCode": "56N"
	},
	{
		"name": "Paul's Letter to Philemon",
		"sortOrder": 87,
		"dbsCode": "PM",
		"usfm": "PHM",
		"osis": "Phlm",
		"chapters": [
			25
		],
		"chapterCount": 1,
		"names": {
			"eng": [
				"Philemon",
				"Pm",
				"Phile",
				"Phile",
				"Philm",
				"Pm"
			]
		},
		"testament": "n",
		"unboundCode": "57N"
	},
	{
		"name": "The Letter to the Hebrews",
		"sortOrder": 88,
		"dbsCode": "HB",
		"usfm": "HEB",
		"osis": "Heb",
		"chapters": [
			14,
			18,
			19,
			16,
			14,
			20,
			28,
			13,
			28,
			39,
			40,
			29,
			25
		],
		"chapterCount": 13,
		"names": {
			"eng": [
				"Hebrews",
				"He",
				"Heb",
				"Hw"
			]
		},
		"testament": "n",
		"unboundCode": "58N"
	},
	{
		"name": "The Letter from James",
		"sortOrder": 89,
		"dbsCode": "JM",
		"usfm": "JAS",
		"osis": "Jas",
		"chapters": [
			27,
			26,
			18,
			17,
			20
		],
		"chapterCount": 5,
		"names": {
			"eng": [
				"James",
				"Jm",
				"Jam",
				"Jas",
				"Ja"
			]
		},
		"testament": "n",
		"unboundCode": "59N"
	},
	{
		"name": "The First Letter from Peter",
		"sortOrder": 90,
		"dbsCode": "P1",
		"usfm": "1PE",
		"osis": "1Pet",
		"chapters": [
			25,
			25,
			22,
			19,
			14
		],
		"chapterCount": 5,
		"names": {
			"eng": [
				"1 Peter",
				"1P",
				"1 Pet",
				"1Pet",
				"IPe",
				"1P",
				"I Peter",
				"I Pet",
				"I Pe"
			]
		},
		"testament": "n",
		"unboundCode": "60N"
	},
	{
		"name": "The Second Letter from Peter",
		"sortOrder": 91,
		"dbsCode": "P2",
		"usfm": "2PE",
		"osis": "2Pet",
		"chapters": [
			21,
			22,
			18
		],
		"chapterCount": 3,
		"names": {
			"eng": [
				"2 Peter",
				"2P",
				"2 Pet",
				"2Pet",
				"2Pe",
				"IIP",
				"II Peter",
				"II Pet",
				"II Pe"
			]
		},
		"testament": "n",
		"unboundCode": "61N"
	},
	{
		"name": "John's First Letter",
		"sortOrder": 92,
		"dbsCode": "J1",
		"usfm": "1JN",
		"osis": "1John",
		"chapters": [
			10,
			29,
			24,
			21,
			21
		],
		"chapterCount": 5,
		"names": {
			"eng": [
				"1 John",
				"1J",
				"1 Jn",
				"1Jn",
				"1 Jo",
				"IJo",
				"I John",
				"I Jo",
				"I Jn"
			]
		},
		"testament": "n",
		"unboundCode": "62N"
	},
	{
		"name": "John's Second Letter",
		"sortOrder": 93,
		"dbsCode": "J2",
		"usfm": "2JN",
		"osis": "2John",
		"chapters": [
			13
		],
		"chapterCount": 1,
		"names": {
			"eng": [
				"2 John",
				"2J",
				"2 Jn",
				"2Jn",
				"2 Jo",
				"IIJo",
				"II John",
				"II Jo",
				"II Jn"
			]
		},
		"testament": "n",
		"unboundCode": "63N"
	},
	{
		"name": "John's Third Letter",
		"sortOrder": 94,
		"dbsCode": "J3",
		"usfm": "3JN",
		"osis": "3John",
		"chapters": [
			14
		],
		"chapterCount": 1,
		"names": {
			"eng": [
				"3 John",
				"3J",
				"3 Jn",
				"3 Jn",
				"3Jn",
				"3 Jo",
				"IIIJo",
				"III John",
				"III Jo",
				"III Jn"
			]
		},
		"testament": "n",
		"unboundCode": "64N"
	},
	{
		"name": "Jude's Letter",
		"sortOrder": 95,
		"dbsCode": "JD",
		"usfm": "JUD",
		"osis": "Jude",
		"chapters": [
			25
		],
		"chapterCount": 1,
		"names": {
			"eng": [
				"Jude",
				"Jude",
				"Jude"
			]
		},
		"testament": "n",
		"unboundCode": "65N"
	},
	{
		"name": "The Revelation to John",
		"sortOrder": 96,
		"dbsCode": "RV",
		"usfm": "REV",
		"osis": "Rev",
		"chapters": [
			20,
			29,
			22,
			11,
			14,
			17,
			17,
			13,
			21,
			11,
			19,
			17,
			18,
			20,
			8,
			21,
			18,
			24,
			21,
			15,
			27,
			20
		],
		"chapterCount": 22,
		"names": {
			"eng": [
				"Revelation",
				"Re",
				"Rev",
				"Rvltn"
			]
		},
		"testament": "n",
		"unboundCode": "66N"
	},
	{
		"name": "Back matter",
		"sortOrder": 97,
		"dbsCode": "BK",
		"usfm": "BAK",
		"osis": "Back",
		"chapters": [],
		"chapterCount": 1,
		"names": {
			"eng": [
				"Back matter"
			]
		},
		"testament": "x"
	},
	{
		"name": "Other",
		"sortOrder": 98,
		"dbsCode": "OH",
		"usfm": "OTH",
		"osis": "Other",
		"chapters": [],
		"chapterCount": 1,
		"names": {
			"eng": [
				"Other"
			]
		},
		"testament": "x"
	},
	{
		"name": "XXA",
		"sortOrder": 99,
		"dbsCode": "XA",
		"usfm": "XXA",
		"osis": "XXA",
		"chapters": [],
		"chapterCount": 151,
		"names": {
			"eng": [
				"XXA"
			]
		},
		"testament": "x"
	},
	{
		"name": "XXB",
		"sortOrder": 100,
		"dbsCode": "XB",
		"usfm": "XXB",
		"osis": "XXB",
		"chapters": [],
		"chapterCount": 151,
		"names": {
			"eng": [
				"XXB"
			]
		},
		"testament": "x"
	},
	{
		"name": "XXC",
		"sortOrder": 101,
		"dbsCode": "XC",
		"usfm": "XXC",
		"osis": "XXC",
		"chapters": [],
		"chapterCount": 151,
		"names": {
			"eng": [
				"XXC"
			]
		},
		"testament": "x"
	},
	{
		"name": "XXD",
		"sortOrder": 102,
		"dbsCode": "XD",
		"usfm": "XXD",
		"osis": "XXD",
		"chapters": [],
		"chapterCount": 151,
		"names": {
			"eng": [
				"XXD"
			]
		},
		"testament": "x"
	},
	{
		"name": "XXE",
		"sortOrder": 103,
		"dbsCode": "XE",
		"usfm": "XXE",
		"osis": "XXE",
		"chapters": [],
		"chapterCount": 151,
		"names": {
			"eng": [
				"XXE"
			]
		},
		"testament": "x"
	},
	{
		"name": "XXF",
		"sortOrder": 104,
		"dbsCode": "XF",
		"usfm": "XXF",
		"osis": "XXF",
		"chapters": [],
		"chapterCount": 151,
		"names": {
			"eng": [
				"XXF"
			]
		},
		"testament": "x"
	},
	{
		"name": "XXG",
		"sortOrder": 105,
		"dbsCode": "XG",
		"usfm": "XXG",
		"osis": "XXG",
		"chapters": [],
		"chapterCount": 151,
		"names": {
			"eng": [
				"XXG"
			]
		},
		"testament": "x"
	},
	{
		"name": "Glossary",
		"sortOrder": 106,
		"dbsCode": "GS",
		"usfm": "GLO",
		"osis": "Glossary",
		"chapters": [],
		"chapterCount": 1,
		"names": {
			"eng": [
				"Glossary"
			]
		},
		"testament": "x"
	},
	{
		"name": "Concordance",
		"sortOrder": 107,
		"dbsCode": "CN",
		"usfm": "CNC",
		"osis": "Conc",
		"chapters": [],
		"chapterCount": 1,
		"names": {
			"eng": [
				"Concordance"
			]
		},
		"testament": "x"
	},
	{
		"name": "Topical Index",
		"sortOrder": 108,
		"dbsCode": "TX",
		"usfm": "TDX",
		"osis": "Topic",
		"chapters": [],
		"chapterCount": 1,
		"names": {
			"eng": [
				"Topical Index"
			]
		},
		"testament": "x"
	},
	{
		"name": "Names Index",
		"sortOrder": 109,
		"dbsCode": "NX",
		"usfm": "NDX",
		"osis": "Name",
		"chapters": [],
		"chapterCount": 1,
		"names": {
			"eng": [
				"Names Index"
			]
		},
		"testament": "x"
	}
]


OT_BOOKS = ["GN","EX","LV","NU","DT","JS","JG","RT","S1","S2","K1","K2","R1","R2","ER","NH","ET","JB","PS","PR","EC","SS","IS","JR","LM","EK","DN","HS","JL","AM","OB","JH","MC","NM","HK","ZP","HG","ZC","ML"];

NT_BOOKS = ["MT","MK","LK","JN","AC","RM","C1","C2","GL","EP","PP","CL","H1","H2","T1","T2","TT","PM","HB","JM","P1","P2","J1","J2","J3","JD","RV"]


AP_BOOKS = ["TB","JT","ED","AE","WS","SR","BR","LJ","S3Y","SN","BL","M1","M2","E1"]

DEFAULT_BIBLE = OT_BOOKS.concat(NT_BOOKS);
APOCRYPHAL_BIBLE = OT_BOOKS.concat(AP_BOOKS, NT_BOOKS);


function getBookInfoByDbsCode(dbsCode) {
	return getBookInfoByProperty('dbsCode', dbsCode);
}

function getBookInfoByUnboundCode(unboundCode) {
	return getBookInfoByProperty('unboundCode', unboundCode);
}
function getBookInfoByUsfmCode(usfmCode) {
	return getBookInfoByProperty('usfm', usfmCode);
}
function getBookInfoByOsisCode(usfmCode) {
	return getBookInfoByProperty('osis', usfmCode);
}

function getBookInfoByProperty(propName, value) {

	//console.log('getBookInfoByUsfmCode', propName, value);

	for (var i=0, il=BOOK_DATA.length; i<il; i++) {
		var bookInfo = BOOK_DATA[i];
		if (bookInfo[propName] == value) {
			// console.log('found', bookInfo[propName], bookInfo.dbsCode);
			return bookInfo;
		}
	}
	return null;
}

function addNames(lang, bookList, namesData) {
	for (var i in bookList) {
		var bookKey = bookList[i],
			names = namesData[i],
			bookInfo = getBookInfoByDbsCode(bookKey);

		if (typeof names === 'string') {
			bookInfo.names[lang] = [names];
		} else {
			bookInfo.names[lang] = names;
		}
	}
}

function getNextChapter(dbsChapterCode, bookCodes) {
	
	if (typeof bookCodes == 'undefined') {
		bookCodes = DEFAULT_BIBLE;		
	}	
	
	var dbsCode = dbsChapterCode.substring(0,2),
		chapter = parseInt(dbsChapterCode.substring(2), 10),
		bookInfo = getBookInfoByDbsCode(dbsCode),
		bookIndex = bookCodes.indexOf(dbsCode);

	if (chapter < bookInfo.chapters.length-1) {
		chapter++
		return dbsCode + chapter.toString();
	} else {
		// next book
		if (bookIndex == bookCodes.length -1) {
			return null;
		} else {
			
			var nextBookCode = bookCodes[bookIndex+1];
			
			// LXX blanks
			if (nextBookCode == '') {
				bookCodes[bookIndex+2];
			}
			
			return nextBookCode + '1';
		}
	}
}

function getPrevChapter(dbsChapterCode, bookCodes) {
	
	if (typeof bookCodes == 'undefined') {
		bookCodes = DEFAULT_BIBLE;		
	}
	
	var dbsCode = dbsChapterCode.substring(0,2),
		chapter = parseInt(dbsChapterCode.substring(2), 10),
		bookInfo = getBookInfoByDbsCode(dbsCode),
		bookIndex = bookCodes.indexOf(dbsCode);

	if (chapter > 1) {
		chapter--;
		return dbsCode + chapter.toString();
	} else {
		// prev book
		if (bookIndex == 0) {
			return null;
		} else {
			var prevBookCode = bookCodes[bookIndex-1];
			
			// for LXX where we currently have empty books
			if (prevBookCode == '') {
				prevBookCode = bookCodes[bookIndex-2];
			}			
			
			var prevBookInfo = getBookInfoByDbsCode(prevBookCode);
			
			return prevBookInfo.dbsCode + prevBookInfo.chapters.length.toString();
		}

	}
}


function getBookName(dbsCode, lang) {
	var bookInfo = getBookInfoByDbsCode(dbsCode),
		name = '';

	if (bookInfo != null && bookInfo.names && bookInfo.names[lang]) {
		name = bookInfo.names[lang];

		if (typeof name == 'string') {
			return name;
		} else {
			return name[0];
		}

	} else {
		return null;
	}
}

// Burmese
addNames("bur",DEFAULT_BIBLE,[
"ကမၻာဦးက်မ္း  ","ထြက္ေျမာက္ရာက်မ္း  ","ဝတ္ျပဳရာက်မ္း  ","ေတာလည္ရာက်မ္း  ","တရားေဟာရာက်မ္း   ","ေယာရႈမွတ္စာ  ","တရားသူႀကီးမွတ္စာ   ","ရုသဝတၳဳ   ","ဓမၼရာဇဝင္ပဌမေစာင္   ","ဓမၼရာဇဝင္ဒုတိယေစာင္   ","ဓမၼရာဇဝင္တတိယေစာင္ ","ဓမၼရာဇဝင္စတုတၳေစာင္   ","ရာဇဝင္ခ်ဳပ္ပဌမေစာင္   ","ရာဇဝင္ခ်ဳပ္ဒုတိယေစာင္   ","ဧဇရမွတ္စာ  ","ေနဟမိမွတ္စာ   ","ဧသတာဝတၱဳ   ","ေယာဘဝတၱဳ   ","ဆာလံက်မ္း   ","သုတၱံက်မ္း   ","ေဒသနာက်မ္း   ","ေရွာလမုန္သီခ်င္း   ","ေဟရွာယအနာဂတိၱက်မ္း   ","ေယရမိအနာဂတၱိက်မ္း   ","ေယရမိျမည္တမ္းစကား   ","ေယဇေက်လအနာကတၱိက်မ္း   ","ဒံေယလအနာဂတၱိက်မ္း   ","ေဟာေရွအနာဂတၱိက်မ္း   ","ေယာလအနာဂတၱိက်မ္း   ","အာမုတ္အနာဂတၱိက်မ္း   ","ၾသဗဒိဗ်ာဒိတ္ရူပါရုံ   ","ေယာနဝတၳဳ   ","မိကၡာအနာဂတၱိက်မ္း   ","နာဟံုအနာဂတၱိက်မ္း   ","ဟဗကၠဳတ္အနာဂတၱိက်မ္း   ","ေဇဖနိအနာဂတၱိက်မ္း   ","ဟဂၢဲအနာဂတၱိက်မ္း   ","ဇာခရိအနာဂတၱိက်မ္း   ","မာလခိအနာဂတၱိက်မ္း   ","ရွင္မႆဲခရစ္ဝင္   ","ရွင္မာကုခရစ္ဝင္   ","ရွင္လုကာခရစ္ဝင္   ","ရွင္ေယာဟန္ခရစ္ဝင္   ","တမန္ေတာ္ဝတၳဳ   ","ေရာမၾသဝါဒစာ   ","ေကာရိ ႏၱဳ ၾသဝါဒစာပဌမေစာင္  ","ေကာရိ သု ၾသဝါဒစာဒုတိယေစာင္   ","ဂလာတိၾသဝါဒစာ   ","ဧဖက္ၾသဝါဒစာ   ","ဖိလိပၸိၾသဝါဒစာ   ","ေကာေလာသဲၾသဝါဒစာ   ","သက္သာေလာနိတ္ၾသဝါဒစာပဌမေစာင္   ","သက္သာေလာနိတ္ၾသဝါဒစာဒုတိယေစာင္   ","တိေမာေသၾသဝါဒပဌမေစာင္   ","တိေမာေသၾသဝါဒစာဒုတိယေစာင္   ","တိတုၾသဝါဒစာ   ","ဖိေလမုန္ၾသဝါဒစာ   ","ေဟျဗဲၾသဝါဒစာ   ","ရွင္ယာကုပ္ၾသဝါဒစာ   ","ရွင္ေပတရုၾသဝါဒစာပဌမေစာင္   ","ရွင္ေပတရုၾသဝါဒစာဒုတိယေစာင္   ","ရွင္ေယာဟန္ၾသဝါဒစာပဌမေစာင္    ","ရွင္ေယာဟန္ၾသဝါဒစာဒုတိယေစာင္    ","ရွင္ေယာဟန္ၾသဝါဒစာတတိယေစာင္   ","ရွင္ယုဒၾသဝါဒစာ   ","ဗ်ာဒိတ္က်မ္း   "
]);

// spanish
addNames("spa", DEFAULT_BIBLE, [
	"Génesis","Exodo","Levítico","Números","Deuteronomio","Josué","Jueces","Rut","1 Samuel","2 Samuel","1 Reyes","2 Reyes","1 Crónicas","2 Crónicas","Esdras","Nehemías","Ester","Job","Salmos","Proverbios","Eclesiastés","Cantares","Isaías","Jeremías","Lamentaciones","Ezequiel","Daniel","Oseas","Joel","Amós","Abdías","Jonás","Miqueas","Nahúm","Habacuc","Sofonías","Hageo","Zacarías","Malaquías",
	"Mateo","Marcos","Lucas","Juan","Hechos","Romanos","1 Corintios","2 Corintios","Gálatas","Efesios","Filipenses","Colosenses","1 Tesalonicenses","2 Tesalonicenses","1 Timoteo","2 Timoteo","Tito","Filemón","Hebreos","Santiago","1 Pedro","2 Pedro","1 Juan","2 Juan","3 Juan","Judas","Apocalipsis"
]);

// French
addNames("fra",DEFAULT_BIBLE,[
	"Genèse","Exode","Lévitique","Nombres","Deutéronome","Josué","Juges","Ruth","1 Samuel","2 Samuel","1 Rois","2 Rois","1 Chroniques","2 Chroniques","Esdras","Néhémie","Esther","Job","Psaumes","Proverbes","Ecclésiaste","Cantique","Esaïe","Jérémie","Lamentations","Ezéchiel","Daniel","Osée","Joël","Amos","Abdias","Jonas","Michée","Nahum","Habacuc","Sophonie","Aggée","Zacharie","Malachie",
	"Matthieu","Marc","Luc","Jean","Actes","Romains","1 Corinthiens","2 Corinthiens","Galates","Ephésiens","Philippiens","Colossiens","1 Thessalonicien","2 Thessalonicien","1 Timothée","2 Timothée","Tite","Philemon","Hébreux","Jacques","1 Pierre","2 Pierre","1 Jean","2 Jean","3 Jean","Jude","Apocalypse"
]);

// German
addNames("de",DEFAULT_BIBLE,[
	"Genesis","Exodus","Levitikus","Numeri","Deuteronomium","Josua","Richter","Rut","1 Samuel","2 Samuel","1 Könige","2 Könige","1 Chronik","2 Chronik","Esra","Nehemia","Ester",["Hiob","Ijob"],"Psalter","Sprichwörter","Kohelet","Hohelied Salmos","Jesaja","Jeremia",["Klagelieder","Jeremias"],["Hesekiel","Ezechiel"],"Daniel","Hosea","Joel","Amos","Obadja","Jona","Micha","Nahum","Habakuk","Zefanja","Haggai","Sacharja","Maleachi",
	"Matthäus","Markus","Lukas","Johannes","Apostelgeschichte des Lukas","Römer","1 Korinther","2 Korinther","Galater","Epheser","Philipper","Kolosser","1 Thessalonicher","2 Thessalonicher","1 Timotheus","2 Timotheus","Titus","Philemon","Hebräer","Jakobus","1 Petrus","2 Petrus","1 Johannes","2 Johannes","3 Johannes","Judas","Offenbarung des Johannes"
]);

// Greek
var greekNames = [
	"ΓΕΝΕΣΙΣ","ΕΞΟΔΟΣ","ΛΕΥΙΤΙΚΟΝ","ΑΡΙΘΜΟΙ","ΔΕΥΤΕΡΟΝΟΜΙΟΝ","ΙΗΣΟΥΣ ΝΑΥΗ","ΚΡΙΤΑΙ","ΡΟΥΘ","ΒΑΣΙΛΕΙΩΝ Α΄","ΒΑΣΙΛΕΙΩΝ B΄","ΒΑΣΙΛΕΙΩΝ Γ΄","ΒΑΣΙΛΕΙΩΝ Δ΄","ΠΑΡΑΛΕΙΠΟΜΕΝΩΝ Α΄","ΠΑΡΑΛΕΙΠΟΜΕΝΩΝ B΄","ΕΣΔΡΑΣ","ΝΕΕΜΙΑΣ","ΕΣΘΗΡ","ΙΩΒ","ΨΑΛΜΟΙ","ΠΑΡΟΙΜΙΑΙ","ΕΚΚΛΗΣΙΑΣΤΗΣ","ΑΣΜΑ ΑΣΜΑΤΩΝ","ΗΣΑΪΑΣ","ΙΕΡΕΜΙΑΣ","ΘΡΗΝΟΙ ΙΕΡΕΜΙΟΥ","ΙΕΖΕΚΙΗΛ ","ΔΑΝΙΗΛ","ΩΣΗΕ","ΙΩΗΛ","ΑΜΩΣ","ΟΒΔΙΟΥ","ΙΩΝΑΣ","ΜΙΧΑΙΑΣ","ΝΑΟΥΜ","ΑΜΒΑΚΟΥΜ","ΣΟΦΟΝΙΑΣ ","ΑΓΓΑΙΟΣ","ΖΑΧΑΡΙΑΣ","ΜΑΛΑΧΙΑΣ",
	"ΜΑΤΘΑΙΟΝ","ΜΑΡΚΟΝ","ΛΟΥΚΑΝ","ΙΩΑΝΝΗΝ","ΠΡΑΞΕΙΣ","ΡΩΜΑΙΟΥΣ","ΚΟΡΙΝΘΙΟΥΣ Α΄","ΚΟΡΙΝΘΙΟΥΣ Β΄","ΓΑΛΑΤΑΣ","ΕΦΕΣΙΟΥΣ","ΦΙΛΙΠΠΗΣΙΟΥΣ","ΚΟΛΟΣΣΑΕΙΣ","ΘΕΣΣΑΛΟΝΙΚΕΙΣ Α΄","ΘΕΣΣΑΛΟΝΙΚΕΙΣ Β΄","ΤΙΜΟΘΕΟΝ Α΄","ΤΙΜΟΘΕΟΝ Β΄","ΤΙΤΟΝ","ΦΙΛΗΜΟΝΑ","ΕΒΡΑΙΟΥΣ","ΙΑΚΩΒΟΥ","ΠΕΤΡΟΥ Α΄","ΠΕΤΡΟΥ Β΄","ΙΩΑΝΝΟΥ Α΄","ΙΩΑΝΝΟΥ Β΄","ΙΩΑΝΝΟΥ Γ΄","ΙΟΥΔΑ","ΑΠΟΚΑΛΥΨΙΣ"
];

addNames("grc", DEFAULT_BIBLE, greekNames);
addNames("gre", DEFAULT_BIBLE, greekNames);

// Hebrew
addNames("heb", OT_BOOKS, [
	"בראשית","שמות","ויקרא","במדבר","דברים","יהושע","שופטים","רות","שמואל א","שמואל ב","מלכים א","מלכים ב","דברי הימים א","דברי הימים ב","עזרא","נחמיה","אסתר","איוב","תהילים","קהלת","קהלת","שיר השירים","ישעה","ירמיה","איכה","יחזקאל","דניאל","הושע","יואל","עמוס","עבדיה","יונה","מיכה","נחום","חבקוק","צפניה","חגי","זכריה","מלאכי"
]);

// Russian
addNames('rus', DEFAULT_BIBLE, [
	"Бытие","Исход","Левит","Числа","Второзаконие","Иисус Навин","Книга Судей","Руфь","1-я Царств","2-я Царств","3-я Царств","4-я Царств","1-я Паралипоменон","2-я Паралипоменон","Ездра","Неемия","Есфирь","Иов","Псалтирь","Притчи","Екклесиаст","Песни Песней","Исаия","Иеремия","Плач Иеремии","Иезекииль","Даниил","Осия","Иоиль","Амос","Авдия","Иона","Михей","Наум","Аввакум","Софония","Аггей","Захария","Малахия",
	"От Матфея","От Марка","От Луки","От Иоанна","Деяния","К Римлянам","1-е Коринфянам","2-е Коринфянам","К Галатам","К Ефесянам","К Филиппийцам","К Колоссянам","1-е Фессалоникийцам","2-е Фессалоникийцам","1-е Тимофею","2-е Тимофею","К Титу","К Филимону","К Евреям","Иакова","1-e Петра","2-e Петра","1-e Иоанна","2-e Иоанна","3-e Иоанна","Иуда","Откровение"
]);

// Chinese (Simplified)
addNames('chi', DEFAULT_BIBLE, [
	"創 世 記","出 埃 及 記","利 未 記","民 數 記","申 命 記","約 書 亞 記","士 師 記","路 得 記","撒 母 耳 記 上","撒 母 耳 記 下","列 王 紀 上","列 王 紀 下","歷 代 志 上","歷 代 志 下","以 斯 拉 記","尼 希 米 記","以 斯 帖 記","約 伯 記","詩 篇","箴 言","傳 道 書","雅 歌","以 賽 亞 書","耶 利 米 書","耶 利 米 哀 歌","以 西 結 書","但 以 理 書","何 西 阿 書","約 珥 書","阿 摩 司 書","俄 巴 底 亞 書","約 拿 書","彌 迦 書","那 鴻 書","哈 巴 谷 書","西 番 雅 書","哈 該 書","撒 迦 利 亞","瑪 拉 基 書","馬 太 福 音","馬 可 福 音","路 加 福 音","約 翰 福 音","使 徒 行 傳","羅 馬 書","歌 林 多 前 書","歌 林 多 後 書","加 拉 太 書","以 弗 所 書","腓 立 比 書","歌 羅 西 書","帖 撒 羅 尼 迦 前 書","帖 撒 羅 尼 迦 後 書","提 摩 太 前 書","提 摩 太 後 書","提 多 書","腓 利 門 書","希 伯 來 書","雅 各 書","彼 得 前 書","彼 得 後 書","約 翰 一 書","約 翰 二 書","約 翰 三 書","猶 大 書","启 示 录"
]);

// Chinese (Traditional)
addNames('cmn', DEFAULT_BIBLE, [
	"創 世 記","出 埃 及 記","利 未 記","民 數 記","申 命 記","約 書 亞 記","士 師 記","路 得 記","撒 母 耳 記 上","撒 母 耳 記 下","列 王 紀 上","列 王 紀 下","歷 代 志 上","歷 代 志 下","以 斯 拉 記","尼 希 米 記","以 斯 帖 記","約 伯 記","詩 篇","箴 言","傳 道 書","雅 歌","以 賽 亞 書","耶 利 米 書","耶 利 米 哀 歌","以 西 結 書","但 以 理 書","何 西 阿 書","約 珥 書","阿 摩 司 書","俄 巴 底 亞 書","約 拿 書","彌 迦 書","那 鴻 書","哈 巴 谷 書","西 番 雅 書","哈 該 書","撒 迦 利 亞","瑪 拉 基 書","馬 太 福 音","馬 可 福 音","路 加 福 音","約 翰 福 音","使 徒 行 傳","羅 馬 書","歌 林 多 前 書","歌 林 多 後 書","加 拉 太 書","以 弗 所 書","腓 立 比 書","歌 羅 西 書","帖 撒 羅 尼 迦 前 書","帖 撒 羅 尼 迦 後 書","提 摩 太 前 書","提 摩 太 後 書","提 多 書","腓 利 門 書","希 伯 來 書","雅 各 書","彼 得 前 書","彼 得 後 書","約 翰 一 書","約 翰 二 書","約 翰 三 書","猶 大 書","启 示 录"
]);

// Arabic
addNames('ara', DEFAULT_BIBLE, [
	"التكوين","الخروج","اللاويين","العدد","التثنية","يشوع","القضاة","راعوث","صموئيل الأول","صموئيل الثاني","الملوك الأول","الملوك الثاني","أخبار الأيام الأول","أخبار الأيام الثاني","عزرا","نحميا","أستير","أيوب","المزامير","الأمثال","الجامعة","نشيد الأنشاد","أشعياء","أرمياء","مراثي أرمياء","حزقيال","دانيال","هوشع","يوئيل","عاموس","عوبديا","يونان","ميخا","ناحوم","حبقوق","صفنيا","حجي","زكريا","ملاخي","متى","مرقس","لوقا","يوحنا","أعمال الرسل","رومية","1 كورنثوس","2 كورنثوس","غلاطية","أفسس","فيليبي","كولوسي","1 تسالونيكي","2 تسالونيكي","1 تيموثاوس","2 تيموثاوس","تيطس","فيليمون","العبرانيين","يعقوب","1 بطرس","2 بطرس","1 يوحنا","2 يوحنا","3 يوحنا","يهوذا","رؤيا"
]);

// turkish
addNames('tur', DEFAULT_BIBLE, [
["Yaratilis", "Tekvin"],
["Misir'dan Çikis", "Çikis"],
"Levililer",
["Çölde Sayim", "Sayilar"],
"Yasa'nin Tekrari",
"Yesu",
"Hakimler",
"Rut",
"1 Samuel",
"2 Samuel",
"1 Krallar",
"2 Krallar",
"1 Tarihler",
"2 Tarihler",
"Ezra",
"Nehemya",
"Ester",
["Eyüp", "Eyub"],
"Mezmurlar",
["Süleyman'in Özdeyisleri", "Süleyman'in Meselleri"],
"Vaiz",
["Ezgiler Ezgisi", "Nesideler Nesidesi"],
["Yesaya", "Isaya"],
"Yeremya",
["Agitlar", "Yeremya'nin Mersiyeleri"],
"Hezekiel",
"Daniel",
"Hosea",
"Yoel",
"Amos",
["Ovadya", "Obadya"],
"Yunus",
"Mika",
"Nahum",
"Habakkuk",
["Sefanya", "Tsefanya"],
["Hagay", "Haggay"],
["Zekeriya", "Zekarya"],
"Malaki",
"Matta",
"Markos",
"Luka",
"Yuhanna",
["Elçilerin Isleri", "Resullerin Isleri"],
"Romalilar",
["1 Korintliler", "1. Korintoslular"],
["2 Korintliler", "2. Korintoslular"],
"Galatyalilar",
["Efesliler", "Efesoslular"],
"Filipililer",
"Koloseliler",
"1 Selanikiler",
"2 Selanikiler",
"1 Timoteos",
"2 Timoteos",
"Titus",
"Filimon",
"Ibraniler",
["Yakup", "Yakub"],
"1 Petrus",
"2 Petrus",
"1 Yuhanna",
"2 Yuhanna",
"3 Yuhanna",
"Yahuda",
"Vahiy"

]);

//Vietnamese
var VietNames = [
	"Sáng Thế Ký","Xuất Ê-díp-tô Ký","Lê-vi Ký","Dân Số Ký","Phục Truyền Luật Lệ Ký","Giô-suê","Các Quan Xét","Ru-tơ","1 Sa-mu-ên","2 Sa-mu-ên","1 Các Vua","2 Các Vua","1 Sử Ký","2 Sử Ký","E-xơ-ra","Nê-hê-mi","Ê-xơ-tê",["Gióp","Gióp"],"Thi Thiên","Châm Ngôn","Truyền Đạo","Nhã Ca","Ê-sai","Giê-rê-mi",["Ca Thương","Ai Ca"],["Ê-xê-chi-ên","Ê-xê-chi-ên"],"Đa-ni-ên","Ô-sê","Giô-ên","A-mốt","Áp-đia","Giô-na","Mi-chê","Na-hum","Ha-ba-cúc","Sô-phô-ni","A-ghê","Xa-cha-ri","Ma-la-chi",
	"Ma-thi-ơ","Mác","Lu-ca","Giăng","Công Vụ Các Sứ Đồ","Rô-ma","1 Cô-rinh-tô","2 Cô-rinh-tô","Ga-la-ti","Ê-phê-sô","Phi-líp","Cô-lô-se","1 Tê-sa-lô-ni-ca","2 Tê-sa-lô-ni-ca","1 Ti-mô-thê","2 Ti-mô-thê","Tít","Phi-lê-môn","Hê-bơ-rơ","Gia-cơ","1 Phi-e-rơ","2 Phi-e-rơ","1 Giăng","2 Giăng","3 Giăng","Giu-đe","Khải Huyền"
];
addNames("vi", DEFAULT_BIBLE, VietNames);
addNames("vni", DEFAULT_BIBLE, VietNames);
addNames("vnm", DEFAULT_BIBLE, VietNames);
addNames("vn_vni2018", DEFAULT_BIBLE, VietNames);
//msToan add names of Bible books in Vietnamese

module.exports = {
	getBookInfoByUnboundCode: getBookInfoByUnboundCode,
	getBookInfoByDbsCode: getBookInfoByDbsCode,
	getBookInfoByUsfmCode: getBookInfoByUsfmCode,
	getBookInfoByOsisCode: getBookInfoByOsisCode,
	getBookName: getBookName,
	getPrevChapter: getPrevChapter,
	getNextChapter: getNextChapter,
	DEFAULT_BIBLE: DEFAULT_BIBLE,
	OT_BOOKS: OT_BOOKS,
	NT_BOOKS: NT_BOOKS,
	AP_BOOKS: AP_BOOKS,
	BOOK_DATA: BOOK_DATA
}
